...
public class Main{
	public static void main(String args[]) throws IOException{
		...
		int serchArea1 = 1;//要救助者のいる領域名1
		int serchArea2 = 2;//要救助者のいる領域名2
		...
		Area position = new Area(peaple, serchArea1, serchArea2, serchCase);
		...
		while(lapseTime < endTime) {
			...
			for(int i = 0; i < 9; i++) {
				...
				if(drone[i].time.equals("sensingstart")) {
					...
					try {//ファイルへの書き込み
						FileWriter fw = new FileWriter("/Users/e1558219/Desktop/sennsingStartTime.txt",true);
				...
				if(drone[i].time.equals("sensingEnd")) {
					String str =  String.valueOf(lapseTime) + " " + String.valueOf(drone[i].id);
					try {//ファイルへの書き込み
						FileWriter fw = new FileWriter("/Users/e1558219/Desktop/sennsingEndTime.txt",true);
						BufferedWriter bw = new BufferedWriter(fw);
						bw.write(str);
						bw.newLine();//改行
						bw.flush();
				...
				if(drone[i].time.equals("missionEnd")) {
					String str =  String.valueOf(lapseTime) + " " + String.valueOf(drone[i].id);
					try {//ファイルへの書き込み
						FileWriter fw = new FileWriter("/Users/e1558219/Desktop/missionEndTime.txt",true);
						BufferedWriter bw = new BufferedWriter(fw);
						bw.write(str);
						bw.newLine();//改行
						bw.flush();
